...
siteNameHead.addEventListener('click', () => {
  makeDataGrid(siteNameHead.name)
});
cityNameHead.addEventListener('click', () => {
  makeDataGrid(cityNameHead.name)
});
...
const getWoeidCity = async (data) => {
  ...
  if (data.location.woeid == '29334827' || !data.location.woeid || data.location.city == "Ca'") {
    ...
    tempNewCity = data.location.city + ',' + data.location.region;
    console.log(tempNewCity, 'temp data')
...
const getStoredWoeid = async (data) => {
  console.log(data, 'accessing server...')
  ...
  console.log(newData, 'Success. Server Closed.')
  return newData;
}
...
const openEdit = async (element, isEdit) => {
  ...
  console.log(addData)
  let editWoeid = await getStoredWoeid({
    city: element.SiteCities
  })
  getWoeidCity({
    woeid: editWoeid.ID
  })
